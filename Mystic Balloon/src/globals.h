...
void loadSetEEPROM()
{
  ...
  {
    EEPROM.put(OFFSET_MYBL_START, GAME_ID); // game id
    EEPROM.put(OFFSET_LEVEL, LEVEL_TO_START_WITH - 1); // beginning level
    EEPROM.put(OFFSET_COINS, 0); // coins current run
    EEPROM.put(OFFSET_COINSHS, 0); // coins highscore run
    EEPROM.put(OFFSET_SCORE, (unsigned long)0); // clear score
    EEPROM.put(OFFSET_HSCORE, (unsigned long)0); // clear high score
    EEPROM.put(OFFSET_MYBL_END, GAME_ID); // game id
  }
}
